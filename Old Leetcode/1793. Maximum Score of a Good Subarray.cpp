...
public:
    int maximumScore(vector<int>& nums, int k) 
    {
        ...
        while (lo >= 0 || hi < nums.size())
        {
            ...
            if (lo >= 1)
            {
                int moveWIthNewMinLeft = INT_MIN;
                int moveWIthOldMinLeft = INT_MIN;
                if (nums[lo - 1] < miniNum)
                {
                    moveWIthNewMinLeft = nums[lo - 1] * (hi - (lo-1) + 1);
                }
                else
                {
                    moveWIthOldMinLeft = miniNum * (hi - (lo - 1) + 1);
                }
                int maxByMovingLeft = max(moveWIthNewMinLeft, moveWIthOldMinLeft);
                if(maxScore < maxByMovingLeft)
                {
                    if (maxByMovingLeft == moveWIthNewMinLeft)
                        miniNum = nums[lo - 1];
                    maxScore = maxByMovingLeft;
                    lo--;
                }
            }
            if (hi < nums.size() - 1)
            {
                int moveRightWithNewMin = INT_MIN;
                int moveRightOldNewMin = INT_MIN;
                if (nums[hi + 1] < miniNum)
                {
                    moveRightWithNewMin = nums[hi+1] * (hi + 1 - lo + 1);
                }
                else
                {
                    moveRightOldNewMin = miniNum * (hi + 1 - lo + 1);
                }                  

                int maxByMovingRight = max(moveRightOldNewMin,moveRightWithNewMin);
                if (maxScore < maxByMovingRight)
                {
                    if (maxByMovingRight == moveRightWithNewMin)
                        miniNum = nums[hi + 1];
                    maxScore = maxByMovingRight;
                    hi++;
                }

            }
            if (oldMaxScore == maxScore)
                break;
        }
        return maxScore;
    }
